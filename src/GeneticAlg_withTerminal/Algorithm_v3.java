...
public class Algorithm_v3 {
    ...
    public Algorithm_v3(double selectionRate, LGraph graph, int populationSize, int generationCount) {
    ...
    private List<Ant> generateParents(){
        int parentNumber = (int) (selectionRate*populationSize)/2;
        List<Ant> potentialParentsThisGen = population.stream().sorted(Comparator.comparing(Ant::getAccCost)).toList();
        List<Ant> potentialParentsFromLegacy = queens.stream().sorted(Comparator.comparing(Ant::getAccCost)).toList();
        List<Ant> parents = new ArrayList<>();
        for (int i = 0; i < Math.min(parentNumber, potentialParentsThisGen.size()-1); i++) {
            parents.add(potentialParentsThisGen.get(i));
        }
        for (int i = 0; i < Math.min(parentNumber, potentialParentsFromLegacy.size()-1); i++) {
            parents.add(potentialParentsFromLegacy.get(i));
        }
